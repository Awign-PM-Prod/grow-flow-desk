...
import { NewSalesOwnerSelect } from "@/components/NewSalesOwnerSelect";
import {
  buildNewSalesOwnerOptions,
...
interface MandateFormData {
  ...
  staffingHeadcount: string;
  staffingSalaryPayouts: string;
  staffingProgramManagement: string;
  staffingSaasUsageFee: string;
  staffingMonthlyAgencyFeePercent: string;
  staffingSalesForceAutomationSetupFee: string;
  staffingRecruitmentCost: string;
  staffingMiscRecurring: string;
...
const calculateRevenueSectionType = (
  ...
  if (normalizedUseCase === "retail branding") {
    if (normalizedSubUseCase === "merchandiser driven programs") return "A";
    if (
      normalizedSubUseCase === "signage deployments" ||
      normalizedSubUseCase === "onetime pos and deployment"
    ) {
      return "B";
    }
  }
...
export default function Mandates() {
  ...
  const [kams, setKams] = useState<
    { id: string; email: string; full_name: string; team: Team | null }[]
  >([]);
  ...
  const createNewSalesOwnerOptions = useMemo(
    () =>
      buildNewSalesOwnerOptions({
        kams,
        nsos,
        mandateTeam: createEffectiveTeam,
      }),
    [kams, nsos, createEffectiveTeam],
  );

  const editNewSalesOwnerOptions = useMemo(
    () =>
      buildNewSalesOwnerOptions({
        kams,
        nsos,
        mandateTeam: editEffectiveTeam,
      }),
    [kams, nsos, editEffectiveTeam],
  );
  ...
  useEffect(() => {
    if (!formData.newSalesOwner) return;
    if (isOwnTeamKamEmail(formData.newSalesOwner, kams, createEffectiveTeam)) {
      setFormData((prev) => ({ ...prev, newSalesOwner: "" }));
    }
  }, [createEffectiveTeam, kams, formData.newSalesOwner]);

  useEffect(() => {
    if (!editMandateData?.newSalesOwner) return;
    if (isOwnTeamKamEmail(editMandateData.newSalesOwner, kams, editEffectiveTeam)) {
      setEditMandateData((prev: any) =>
        prev ? { ...prev, newSalesOwner: "" } : prev,
      );
    }
  }, [editEffectiveTeam, kams, editMandateData?.newSalesOwner]);
  ...
  useEffect(() => {
    if (editMandateData?.lob && !allowedLobOptions.includes(editMandateData.lob)) {
      setEditMandateData((prev: any) => ({
        ...prev,
        lob: "",
        useCase: "",
  ...
  }, [allowedLobOptions, editMandateData?.lob]);
    ...
    try {
      ...
      if (showStaffingMandateFields && staffingRevenueSectionType === "A") {
        const requiredStaffingFields = [
          ...
          formData.staffingProgramManagement,
          formData.staffingSaasUsageFee,
          formData.staffingMonthlyAgencyFeePercent,
          formData.staffingSalesForceAutomationSetupFee,
          formData.staffingRecruitmentCost,
    ...
    try {
      ...
      for (let i = 0; i < mandatesToInsert.length; i += batchSize) {
        ...
        batch.forEach((mandate: any) => {
          ...
          if (existingId) {
            // Update existing
            toUpdate.push({ ...mandateWithMappedTeam, id: existingId });
          } else {
            // Insert new
            toInsert.push(mandateWithMappedTeam);
          }
        });
  ...
  const filteredMandates = mandates.filter((mandate) => {
    ...
    const matchesTeam = filterTeam === "all" || mandate.team === filterTeam;
                ...
                <CardContent className="pt-6">
                  ...
                  {showStaffingMandateFields && (
                    <div className="space-y-2">
                      ...
                      <Input
                        id="revenueSectionType"
                        value={staffingRevenueSectionType}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2 md:col-span-2">
                      ...
                      <NewSalesOwnerSelect
                        ...
                        options={createNewSalesOwnerOptions}
                        kams={kams}
                        nsos={nsos}
                        open={nsoSelectOpen}
                        onOpenChange={setNsoSelectOpen}
                        onChange={(email) => handleInputChange("newSalesOwner", email)}
                      />
                    </div>
                    )}
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      {isEditMode ? (
                        <NewSalesOwnerSelect
                          ...
                          options={editNewSalesOwnerOptions}
                          kams={kams}
                          nsos={nsos}
                          open={editNsoSelectOpen}
                          onOpenChange={setEditNsoSelectOpen}
                          onChange={(email) =>
                            setEditMandateData((prev: any) =>
                              prev ? { ...prev, newSalesOwner: email } : prev,
                            )
                          }
                        />
                      ) : (
                        <p className="mt-1">
                          {selectedMandate.new_sales_owner
                            ? formatNewSalesOwnerLabelFromPeople(
                                selectedMandate.new_sales_owner,
                                kams,
                                nsos,
